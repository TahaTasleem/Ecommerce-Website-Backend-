...
router.get("/:id", protect, (req, res) => {
  ...
  pool.query(orderItemsQuery, [orderId], (error, result1, fields) => {
    if (error) {
      res.status(404).json({
      ...
      orderItems = result1;
      // res.json(results);
      pool.query(userDetailQuery, [orderId], (error, result2) => {
        if (error) {
          res.status(404).json({
          ...
          user = result2[0];
          pool.query(shippingDetailsQuery, [orderId], (error, result3) => {
            if (error) {
              res.status(404).json({
                message: "No ShippingDetails Found",
              });
            } else {
              shippingAddress = result3[0];
              pool.query(paymentDetailsQuery, [orderId], (error, result4) => {
                if (error) {
                  res.status(404).json({
                    message: "No order Details Found",
                  });
                } else {
                  paymentDetails = result4[0];
                  res.status(201).json({
                    id: orderId,
                    orderItems: orderItems,
                    shippingAddress,
